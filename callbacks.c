...
cb_node_remove(struct callback_node *node)
{
    ...
    if (node->directory == NULL &&
        ...
        node->exact == NULL)
    {
        printf("No callbacks left on node %s\n", node->hashtree_node.key);
    ...
    {
    	printf("leaving node %s in place, callbacks remain\n", node->hashtree_node.key);
    }
}
...
cb_gather_search (struct callback_node *node, GList *callbacks_so_far, const char *path)
{
    ...
    if (strlen(path) == 0 || strcmp(path, "/") == 0)
    {
        ...
        for(GList *iter = children; iter; iter=iter->next)
        {
            ...
            {
                callbacks_so_far = g_list_append (callbacks_so_far, g_strdup(child->hashtree_node.key));
            }
        }
...
{
    g_list_foreach(node->following,(GFunc) cb_ref, NULL);
    callbacks_so_far = g_list_concat(callbacks_so_far, g_list_copy(node->following));
    /* Terminating condition */
    if (strlen(path) == 0 || !strchr(path+1, '/'))
    {
        g_list_foreach(node->directory,(GFunc) cb_ref, NULL);
        callbacks_so_far = g_list_concat(callbacks_so_far, g_list_copy(node->directory));

        struct hashtree_node *next_stage = hashtree_path_to_node(&node->hashtree_node, "/*");
        if (next_stage)
        {
            g_list_foreach(((struct callback_node *)next_stage)->following, (GFunc)cb_ref, NULL);
            callbacks_so_far = g_list_concat(callbacks_so_far, g_list_copy(((struct callback_node *)next_stage)->following));
        }

        node = (struct callback_node *)hashtree_path_to_node(&node->hashtree_node, path);
        if (node)
        {
            g_list_foreach(node->exact, (GFunc)cb_ref, NULL);
            callbacks_so_far = g_list_concat(callbacks_so_far, g_list_copy(node->exact));
        }
    ...
    if (next_stage)
    {
       callbacks_so_far = cb_gather ((struct callback_node *)next_stage, callbacks_so_far, path + strlen(tmp) + 1);
    }
    ...
    if (strlen(tmp) > 0)
    {
        ...
        if (next_stage)
        {
            callbacks_so_far = cb_gather((struct callback_node *)next_stage, callbacks_so_far, path + strlen(with_leading_slash));
        }
        free(with_leading_slash);
    }
...
cb_tree_destroy (struct callback_node *node)
{
     ...
     * to pass a copy.
     */
//    if (g_list_length(node->directory) + g_list_length(node->exact) + g_list_length(node->following) > 0)
//        printf("%p: %d / %d / %d\n", node, g_list_length(node->directory),g_list_length(node->exact),
//               g_list_length(node->following));
